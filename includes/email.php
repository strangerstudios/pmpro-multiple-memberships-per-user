...
function pmprommpu_send_checkout_emails($user_id, $checkout_id = -1) {
	...
	if($checkout_id>0) {
		...
		$pmproemail->subject = sprintf(__("Your membership confirmation for %s", 'paid-memberships-pro'), get_option("blogname"));
...
// 				$pmproemail->template = "checkout_trial";
		...
		$pmproemail->subject = sprintf(__("Member Checkout at %s", 'paid-memberships-pro'), get_option("blogname"));
