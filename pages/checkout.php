...
if ( empty( $pmpro_checkout_level_ids ) ) {
	...
	$implosion = implode( '+', $pmpro_checkout_level_ids );


if ( ! empty( $_REQUEST['review'] ) ) {
	$action = pmpro_url( 'checkout', '?level=' . $pmpro_checkout_level_ids );
}

if ( isset( $_REQUEST['dellevels'] ) ) {
	$levelstodel = esc_attr( $_REQUEST['dellevels'] );
} else {
	$levelstodel = null;
}
?>
<div id="pmpro_level-mmpu">
<form id="pmpro_form" class="pmpro_form" action="<?php echo $action; ?>" method="post">
	<input type="hidden" id="level" name="level" value="<?php echo $implosion; ?>" />
	<input type="hidden" id="levelstodel" name="levelstodel" value="<?php echo $levelstodel; ?>" />
		...
		<tr>
			<th>
				...
				if ( count( $pmpro_levels ) > 1 ) {
					?>
					<span class="pmpro_thead-msg"><a href="<?php echo pmpro_url( 'levels' ); ?>"><?php _e( 'change', 'paid-memberships-pro' ); ?></a></span><?php } ?>
			</th>
		</tr>
	</thead>
	<tbody>
	...
	<thead>
		<tr>
			<th>
				<span class="pmpro_thead-name"><?php _e( 'Account Information', 'paid-memberships-pro' ); ?></span>
				<span class="pmpro_thead-msg"><?php _e( 'Already have an account?', 'paid-memberships-pro' ); ?> <a href="<?php echo wp_login_url( pmpro_url( 'checkout', '?level=' . $checkout_levels ) ); ?>"><?php _e( 'Log in here', 'paid-memberships-pro' ); ?></a></span>
			</th>
		</tr>
	</thead>
	<tbody>
	...
	<?php } elseif ( $current_user->ID && ! $pmpro_review ) { ?>
		...
		<p id="pmpro_account_loggedin">
			<?php printf( __( 'You are logged in as <strong>%1$s</strong>. If you would like to use a different account for this membership, <a href="%2$s">log out now</a>.', 'paid-memberships-pro' ), $current_user->user_login, wp_logout_url( $_SERVER['REQUEST_URI'] ) ); ?>
		</p>
	<?php } ?>
